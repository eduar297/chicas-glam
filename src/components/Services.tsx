...
const Services = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<string>('Cejas');

  useEffect(() => {
    // Importar los servicios desde el archivo JSON
    import('../data/services.json').then((data) => {
      setServices(data.default);
      if (data.default.length > 0) {
        setSelectedCategory(data.default[0].category);
      }
    });
  }, []);
  ...
  return (
    <section id="services" className="w-full py-12 md:py-16 lg:py-20 bg-gradient-to-b from-white to-nude-50">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12 md:mb-16">
          ...
          <p className="text-base md:text-xl text-gray-600 max-w-2xl mx-auto font-modern px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {services
            ...
            ?.services.map((service) => (
              <div
                key={service.id}
                className="bg-white rounded-xl md:rounded-2xl p-4 md:p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 md:hover:-translate-y-2 border border-gray-100"
              >
                <div className="flex justify-between items-start mb-3 md:mb-4">
                  <div className="flex-1">
                    <h3 className="text-base md:text-lg font-semibold text-gray-800 font-modern mb-1 md:mb-2">
                      {service.name}
                    </h3>
                  ...
                  <span className="text-lg md:text-2xl ml-2">{getCategoryIcon(selectedCategory)}</span>
                </div>
        ...
        <div className="text-center mt-12 md:mt-16">
          <div className="bg-gradient-to-r from-rose-500 to-rose-600 rounded-xl md:rounded-2xl p-6 md:p-8 text-white">
            <h3 className="text-xl md:text-2xl font-bold mb-3 md:mb-4">¿No encuentras lo que buscas?</h3>
